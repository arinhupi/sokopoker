...
std::string PokerHand::printHand() const {
	std::string scoreStr;
...
void PokerHand::printScore() const{
...
int PokerHand::calcScoreOfHand(const PlayerCards& plCards, int firstCardIndex){
...
int PokerHand::maxOfThree(int int1, int int2, int int3) const{
...
std::string PokerHand::hexToCardVal(int val) const {
	char ts[3];
	if (val == 0x0a)
		return "10";
	else if (val == 0x0b)
...
int PokerHand::convertToScore(int handVal, int firstVal, int secVal) const{
	return handVal + 0x10 * firstVal + secVal;
}

void PokerHand::breakupScore(int& hand, int& firstVal, int& secVal) const{
	hand = score & 0xf00;
