...
void PlayerCards::dispCards3(bool firstHidden) const {
	std::string frameString = "+";
	for (unsigned int i=0; i < pCards.size(); i++)
		frameString += "---+";
	std::cout << frameString << "\n";
...
int PlayerCards::getCardAmount() const {
	return pCards.size();
}

Card PlayerCards::getCardAt(int cardNr) const {
	return pCards.at(cardNr);
}
